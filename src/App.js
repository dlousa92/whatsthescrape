...
class App extends React.Component {
  ...
  setImagesOnSite(images) {
    ...
    function removeDuplicates(imagesOnSiteWithDuplicates, src) {
      ...
      for (var i in imagesOnSiteWithDuplicates) {
        lookupObject[imagesOnSiteWithDuplicates[i][src]] =
          imagesOnSiteWithDuplicates[i];
      }
  ...
  handleSubmit(e) {
    ...
    axios
      .all([
        axios.get("https://whatsthescrape.herokuapp.com/scrape", {
          params: { url: this.state.url },
        }),
        axios.get("https://whatsthescrape.herokuapp.com/scrape-img", {
          params: { url: this.state.url },
        }),
      ])
      .then(
  ...
  displayResults() {
    ...
    return (
      <div>
        ...
        <Chart
          url={this.state.url}
          topTenWords={this.state.topTenWords}
          wordTotal={this.state.wordTotal}
        />
      </div>
    );
  }
